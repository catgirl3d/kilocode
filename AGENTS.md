...
- **Fork Rebase Protocol**:
  - When rebasing on `upstream/main`, strictly follow `FORK_REBASE.md`. Reuse package-scoped executor sessions and classify each stop as Mechanical, Bounded, or Deep. Mechanical stops may continue autonomously; Bounded candidates and implemented Deep decisions require coordinator approval before continue. After any Bounded or Deep resolution, run the two specified final reviewers only after deterministic validation; do not launch them for conflict-free or Mechanical-only rebases. Do not run a default i18n scan.
