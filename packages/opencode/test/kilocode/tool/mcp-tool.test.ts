...
it.instance(
  "asks before connecting and reports next step",
  () =>
    Effect.gen(function* () {
      const asks: Parameters<Tool.Context["ask"]>[0][] = []
      const tool = yield* McpTool.pipe(Effect.flatMap((info) => info.init()))
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(result.output).not.toContain("native tools become available on the next step")
    }),
  {
    config: {
      mcp: {
...
it.instance(
  "disconnects an on-demand server and removes its native tools",
  () =>
    Effect.gen(function* () {
      const asks: Parameters<Tool.Context["ask"]>[0][] = []
      const mcp = yield* MCP.Service
