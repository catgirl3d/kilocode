...
export const McpTool = Tool.define<typeof Params, Meta, MCP.Service | Config.Service, "mcp">(
  ...
  Effect.gen(function* () {
    ...
    return {
      description:
        "Use list to report every known MCP server and its status. Use connect only for servers configured with on_demand: true. After a successful connect, inspect the next step's available tools: use `execute` to discover and call this server's permitted tools when offered, otherwise use only directly exposed MCP tools. Use disconnect to release a connected on-demand server and remove its tools from later steps. Unknown, incomplete, disabled, or non-on-demand servers return clear errors.",
      parameters: Params,
      execute: (params, ctx) =>
        Effect.gen(function* () {
            ...
            return yield* locks.withLock(name)(
              Effect.gen(function* () {
                ...
                return {
                  ...
                  output: `MCP server "${name}" is disconnected. Its tools are no longer available on later steps.`,
          ...
          return yield* locks.withLock(name)(
            Effect.gen(function* () {
              ...
              if (current.status === "connected") {
                return {
                  title: "MCP connected",
                  output: `MCP server "${name}" status: connected. Check the next step for tools from this server.`,
                  metadata: { action: "connect", ok: true, server: name, status: "connected" } satisfies Meta,
                }
              }
