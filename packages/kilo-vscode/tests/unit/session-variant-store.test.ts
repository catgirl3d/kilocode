...
describe("per-session variant selection", () => {
  ...
  it.each([
    ...
    [legacyVariantKey(model), "max"],
  ])("resolves remembered key %s against the configured variant", (key, expected) => {
    const store = { [key]: "high" }
    expect(getVariant(store, model, ["high", "max"], "code", "pending-new", "max")).toBe(expected)
    expect(getAgentVariant(store, model, { variants: { high: {}, max: {} } }, "code", "max")).toBe(expected)
  })
